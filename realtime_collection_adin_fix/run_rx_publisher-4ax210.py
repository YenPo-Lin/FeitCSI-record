...
def create_publisher_callback_picoscenes(nicName, publisher, topic, tx_mac):
    def py_call_back_dump(frame):
        ...
        print(f"[DEBUG] callback NIC={nicName}, addr2={mac_str}, target={tx_mac}", flush=True)
...
def run_publisher_picoscenes(args, publisher, sess_listener: Optional[SessionBusListener]):
    ...
    for idx, nicName in enumerate(nicNames):
        ...
        print(f"[Publisher] Starting RX service for NIC={nicName} topic={args.topic}.{nicID} ...")
        try:
            nic.startRxService()
        except Exception as e:
            print(
                f"[Publisher] Could not start NIC={nicName}: {e}\n"
                f"[Publisher] Continuing with {len(nics)} successfully started NIC(s).",
                flush=True,
            )
            break
        nics[nicName] = nic
        nic_ids[nicName] = nicID
        time.sleep(args.nic_start_interval_sec)

    if not nics:
        raise RuntimeError("No NIC RX service could be started.")
    ...
    for nicName, nic in nics.items():
        nicID = nic_ids[nicName]
        nic.registerGeneralHandler(f"rx_zmq_callback_{nicID}", pub_cbs[nicName])
    ...
    try:
        ...
        for nicName, nic in nics.items():
            try:
                ...
                nic.stopRxService()
                time.sleep(args.nic_stop_interval_sec)
            except Exception:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--startup-wait-sec", dest="startup_wait_sec", type=float, default=0.5,
                        help="Wait time after picoscenes_start() before starting NIC Rx services.")
    parser.add_argument("--nic-start-interval-sec", dest="nic_start_interval_sec", type=float, default=0.15,
                        help="Delay between starting adjacent NIC Rx services.")
    parser.add_argument("--nic-stop-interval-sec", dest="nic_stop_interval_sec", type=float, default=0.05,
                        help="Delay between stopping adjacent NIC Rx services.")
