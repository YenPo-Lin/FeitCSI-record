...
if sys.platform.startswith("linux"):
    library_paths = [
        "/usr/local/PicoScenes/pslib",
        "/usr/local/PicoScenes/lib",
    ]
    for library_path in library_paths:
        ...
        raise OSError(
            "PicoScenes library directory not found; checked: "
            + ", ".join(library_paths)
        )
...
if sys.platform.startswith("linux"):
    cppyy.add_include_path("/usr/local/PicoScenes/include/")
elif sys.platform.startswith('win32'):
    cppyy.I("C:\\Program Files\\PicoScenes\\include")
else:
    raise RuntimeError("Please add PicoScenes include path here!")

# PicoScenes 2025.1217 includes Boost.Process and Boost.Test through its common
# header even though PyPicoScenes does not use them. Cling cannot parse the
# installed Boost.Process/Asio combination, so skip those umbrella headers.
cppyy.cppdef("""
